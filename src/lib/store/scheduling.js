...
import { appointmentSchedule, fromDatetimeLocalValue } from '../appointmentUtils'
...
export function getUpcomingAppointments(userId, myWorkplace, options = {}) {
...
export function saveAppointment(payload, userId) {
  ...
  if (payload.id) {
    ...
    db.appointments[idx] = {
      ...
      assigned_therapist: clinicianProfile?.full_name?.split(' ')[0]
  ...
  const created = {
    ...
    assigned_therapist: clinicianProfile?.full_name?.split(' ')[0] || 'Clinician',
