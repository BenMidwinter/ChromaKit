...
describe('getUpcomingAppointments', () => {
  it('excludes cancelled sessions and past dates', () => {
    ...
    const upcoming = getUpcomingAppointments('user-daniel', myWorkplace)
  ...
  it('limits caseload appointments to the user workplace clients', () => {
    const myWorkplace = { id: 'wp-chroma', role: 'clinical_lead' }
    const caseloadIds = new Set(getClientsForUser('user-daniel', myWorkplace).map(c => c.id))
    const upcoming = getUpcomingAppointments('user-daniel', myWorkplace)
    upcoming.forEach((appt) => {
  ...
  it('returns organisation-wide appointments for service lead scope', () => {
    ...
    const caseload = getUpcomingAppointments('user-ben', myWorkplace, { organisationWide: false })
    const orgWide = getUpcomingAppointments('user-ben', myWorkplace, { organisationWide: true })
