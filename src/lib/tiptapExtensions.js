...
export const SignatureBlock = Node.create({
  ...
  renderHTML({ node, HTMLAttributes }) {
    const lines = [
      node.attrs.signatureText,
      node.attrs.name,
      [node.attrs.title, node.attrs.hcpc ? `HCPC ${node.attrs.hcpc}` : ''].filter(Boolean).join(' · '),
    ].filter(Boolean)

    return [
      'div',
      mergeAttributes(HTMLAttributes, { 'data-signature-block': '', class: 'doc-signature' }),
      ...lines.map(line => ['div', { class: 'doc-signature__line' }, line]),
    ]
  },
